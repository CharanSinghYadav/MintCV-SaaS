...
const allowedOrigins = [
    ...
    process.env.FRONTEND_URL // Tera live domain (e.g. https://mintcv.vercel.app)
].filter(Boolean);

app.use(cors({
    origin: (origin, callback) => {
        // !origin allows mobile apps or Postman testing in dev
        if (!origin || allowedOrigins.includes(origin)) {
            callback(null, true);
        } else {
            callback(new Error("🚫 CORS Gateway Blocked: Origin unauthorized"));
        }
    },
    credentials: true
}));
...
const globalLimiter = rateLimit({
    ...
    legacyHeaders: false,
    message: { success: false, message: "Too many requests from this IP. Take a 15-minute breather! 🧘‍♂️" }
});
...
app.use((req, res, next) => {
    res.status(404).json({ success: false, message: `Route ${req.originalUrl} does not exist on MintCV Fortress.` });
});

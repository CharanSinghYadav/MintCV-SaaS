...
export const generateInterviewPrep = async (req, res) => {
  try {
    ...
    if (error.statusCode === 429) {
      return res.status(429).json({ success: false, message: "AI Server is taking a quick breathing break 🧘‍♂️. Hold tight!" });
    }
...
export const uploadResumeFile = async (req, res) => {
  ...
  try {
    ...
    if (error.statusCode === 429) {
      return res.status(429).json({
        success: false,
        message: "AI Server is taking a quick breathing break 🧘‍♂️. Please try again in a moment!",
        isAiOverloaded: true
      });
    }
    return res.status(500).json({ success: false, message: error.message || "Internal server error occurred." });
  } finally {
    // 🔥 FIX: SERVER MEMORY LEAK CLEANER. Cloudinary ke baad local storage se PDF udao!
...
export const enhanceResumeText = async (req, res) => {
  try {
    ...
    if (error.statusCode === 429) {
      return res.status(429).json({
        success: false,
        message: "AI Server is taking a break 🧘‍♂️. Please try again in a moment!",
        isAiOverloaded: true
      });
    }
    return res.status(500).json({ success: false, message: error.message || "Internal server error occurred." });

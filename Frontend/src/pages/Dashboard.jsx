...
const Dashboard = () => {
  ...
  const handleFileUpload = async (e) => {
    ...
    try {
      ...
      if (error.response?.status === 429) {
          toast.error("AI Server is overloaded! Please wait a minute. 🧘‍♂️", { id: toastId });
      } else if (error.response?.data?.requiresUpgrade) {
        // 🌟 FIX: Specific granular message for PDF upload limit
        toast.error("You've used your 1 free PDF import for today! 📄", { id: toastId });
        openPaywall(); 
      } else {
  ...
  // 🌟 FIX: Granular Toasts for Analytics and Interview Navigation
  const handleScoreNavigation = (resumeId) => {
    navigate(`/analytics/${resumeId}`);
  };

  const handlePrepNavigation = (resumeId) => {
              ...
              <div key={resume._id} className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl overflow-hidden hover:shadow-lg transition-all flex flex-col justify-between group relative">
                ...
                <div className="p-5 flex-1 flex flex-col justify-between">
                  ...
                  <div className="grid grid-cols-2 gap-2 mt-auto pt-4">
                    ...
                    <button onClick={() => handleScoreNavigation(resume._id)} className="flex justify-center items-center gap-1.5 text-[11px] font-bold bg-emerald-50 hover:bg-emerald-100 dark:bg-emerald-500/10 dark:hover:bg-emerald-500/20 text-emerald-600 dark:text-emerald-400 py-2.5 rounded-xl transition-colors cursor-pointer">
                      <Sparkles size={14} /> Score
                    </button>
                    <button onClick={() => handlePrepNavigation(resume._id)} className="flex justify-center items-center gap-1.5 text-[11px] font-bold bg-teal-50 hover:bg-teal-100 dark:bg-teal-500/10 dark:hover:bg-teal-500/20 text-teal-600 dark:text-teal-400 py-2.5 rounded-xl transition-colors cursor-pointer">
